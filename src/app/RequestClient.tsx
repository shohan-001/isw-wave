...
export function RequestClient({ eventName }: { eventName: string }) {
  ...
  // Remember the requester's name across submissions within the session.
  useEffect(() => {
    try {
      const saved = localStorage.getItem("isw_name");
  ...
  // Poll my-requests every 5s so status (pending→approved/played) stays fresh.
  ...
  // We debounce so that hitting Enter/typing quickly can't fire mid-word, but
  // the network call only ever happens via runSearch (button or Enter), never
  // per-keystroke — this protects the YouTube quota (100 units/search).
  ...
  const onSubmitSearch = (e: React.FormEvent) => {
    ...
    // 500ms debounce before the actual call fires.
  ...
  async function submitRequest() {
    ...
    setSubmitError(null);
    try {
      localStorage.setItem("isw_name", trimmed);
    } catch {
      /* ignore */
    }
    ...
    try {
      ...
      // Success — confirmation animation, then reset the flow.
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col px-4 pb-24 pt-6">
      ...
      {/* Search */}
      <form onSubmit={onSubmitSearch} className="sticky top-3 z-10">
              ...
              results.map((r) => (
                ...
                >
                  <button
                    ...
                    className="flex w-full items-center gap-3 rounded-2xl border border-white/5 bg-surface/60 p-2.5 text-left transition active:scale-[0.99] hover:border-wave/40"
                  >
  ...
  return (
    <ul className="flex flex-col gap-2">
      {Array.from({ length: 6 }).map((_, i) => (
        <li
          key={i}
          className="flex items-center gap-3 rounded-2xl border border-white/5 bg-surface/60 p-2.5"
        >
        ...
        <>
          ...
          >
            ...
            <input
              ...
              className="mt-2 w-full rounded-xl border border-white/10 bg-surface px-4 py-3 text-base text-white placeholder:text-white/30 focus:border-wave/50 focus:outline-none"
            />
