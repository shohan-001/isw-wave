...
export async function POST(req: Request) {
  ...
  const event = await prisma.event.findUnique({
    ...
    select: { currentRequestId: true, suspended: true, suspendReason: true },
  });
  if (event?.suspended) {
    return NextResponse.json(
      {
        error: event.suspendReason?.trim()
          ? `This event is suspended: ${event.suspendReason.trim()}`
          : "This event is temporarily suspended.",
        suspended: true,
      },
      { status: 403 }
    );
