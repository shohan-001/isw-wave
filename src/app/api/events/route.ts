...
export async function POST(req: Request) {
  ...
  // Approved organizers get a cap so a single account can't spin up unlimited
  // events against the shared YouTube quota. 0 = unlimited (pre-cap accounts).
  const account = await prisma.user.findUnique({
    where: { id: admin.id },
    select: { eventLimit: true },
  });
  const eventLimit = account?.eventLimit ?? 0;
  if (eventLimit > 0) {
    const existing = await prisma.event.count({ where: { adminId: admin.id } });
    if (existing >= eventLimit) {
      return NextResponse.json(
        {
          error: `You've reached your limit of ${eventLimit} event${
            eventLimit === 1 ? "" : "s"
          }. Contact the ISW Wave team to raise it.`,
        },
        { status: 403 }
      );
    }
